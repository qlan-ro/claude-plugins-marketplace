...
- When starting a feature: Ralph parses `.project-studio/features/*/PRD.md` → `prd.json`
- When completing a feature: archive to `.project-studio/archive/`
...
The `/start-ralph` command generates a project-specific `ralph.sh` wrapper that:
...
```bash
# .ralph/run.sh (generated by /start-ralph)
#!/bin/bash
...
### /start-ralph Command Updates (Extended)
...
- `/start-ralph` command can invoke Ralph's parser to generate `prd.json`

### 2. Ralph Activation Command (NEW)

**Command:** `/start-ralph [feature-id]`
...
| A2 | Create `/start-ralph` command (uses Ralph's parser) | `commands/start-ralph.md` | Medium |
| A3 | Create `/archive-feature` command | `commands/archive-feature.md` | Low |
...
| C3 | `/connect` and `/disconnect` commands | `commands/connect.md` | Low |
...
3. **Start command:** Run `/start-ralph 01-auth` → verify prd.json created at root
4. **Archive command:** Run `/archive-feature` → verify files moved to `.project-studio/archive/`
...
1. **Connection:** Run `/connect` → verify WebSocket connects to companion app
...
> /archive-feature
  ...
  # Ready for next feature: /start-ralph 02-dashboard
```
