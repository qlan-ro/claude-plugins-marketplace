...
ensure_state_dir() {
    if [ ! -d "$STATE_DIR" ]; then
        mkdir -p "$STATE_DIR"
        echo "📁 Created .project-studio directory"
    fi
}
...
EOF
        echo "📋 Initialized orchestration state file"
    fi
}
...
update_yaml_value() {
    ...
    if has_yq; then
        ...
        echo "⚠️  yq not found - state update may be incomplete"
...
case "$ACTION" in
    ...
    "gate-check")
        ...
        if [ "$PASSED" = "true" ]; then
            echo "✅ Gate check passed: $PHASE"
        else
            echo "❌ Gate check failed: $PHASE"
        fi
        ;;
    ...
    "agent-complete")
        ...
        echo "✅ Agent completed: $AGENT -> $OUTPUT_FILE"
        ;;
    ...
    "workflow-type")
        ...
        echo "📋 Workflow type set: $WORKFLOW_TYPE"
        ;;
    ...
    "feature-update")
        ...
        echo "📦 Feature updated: $FEATURE_ID -> $STATUS"
        ;;
    ...
    "add-decision")
        ...
        echo "❓ Decision added: $DECISION_ID"
        ;;
    ...
    "resolve-decision")
        ...
        echo "✅ Decision resolved: $DECISION_ID"
        ;;

    "status")
        # Print current state summary
        if [ -f "$STATE_FILE" ]; then
            echo "📊 Orchestration State Summary"
            echo "==============================="
